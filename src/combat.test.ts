...
describe("Combat Progression", () => {
  ...
  describe("Combat failure does NOT relocate", () => {
    it("should NOT relocate player on combat failure", () => {
      const state = createToyWorld("fight-fail-no-relocate")
      state.player.location = "MINE"
      state.player.skills.Combat = { level: 1, xp: 0 }
      state.player.inventory.push({ itemId: "CRUDE_WEAPON", quantity: 1 })
      state.player.equippedWeapon = "CRUDE_WEAPON"
      ...
      if (log.failureType === "COMBAT_FAILURE") {
        // Player should stay at MINE, not relocate to TOWN
    ...
    it("should consume time on combat failure", () => {
      const state = createToyWorld("fight-fail-time")
      state.player.location = "MINE"
      state.player.skills.Combat = { level: 1, xp: 0 }
      state.player.inventory.push({ itemId: "CRUDE_WEAPON", quantity: 1 })
      state.player.equippedWeapon = "CRUDE_WEAPON"
  ...
  describe("Combat loot table", () => {
    it("should have 10% chance to drop IMPROVED_WEAPON on successful fight", () => {
      ...
      const state = createToyWorld("test-improved-drop")
      state.player.location = "MINE"
      state.player.skills.Combat = { level: 1, xp: 0 }
      state.player.inventory.push({ itemId: "CRUDE_WEAPON", quantity: 1 })
      state.player.equippedWeapon = "CRUDE_WEAPON"
      ...
      if (log.success) {
        // Should have made 2 RNG rolls: fight success + improved weapon drop
        expect(log.rngRolls.length).toBeGreaterThanOrEqual(2)
        const weaponRoll = log.rngRolls.find((r) => r.label.includes("improved-weapon"))
        expect(weaponRoll).toBeDefined()
        expect(weaponRoll?.probability).toBe(0.1)
      }
    })

    it("should have 1% chance to drop COMBAT_GUILD_TOKEN on successful fight", () => {
      const state = createToyWorld("test-token-drop")
      state.player.location = "MINE"
      state.player.skills.Combat = { level: 1, xp: 0 }
      state.player.inventory.push({ itemId: "CRUDE_WEAPON", quantity: 1 })
      state.player.equippedWeapon = "CRUDE_WEAPON"
      ...
      if (log.success) {
        // Should have made 3 RNG rolls: fight success + improved weapon + token
        expect(log.rngRolls.length).toBeGreaterThanOrEqual(3)
        const tokenRoll = log.rngRolls.find((r) => r.label.includes("combat-token"))
        expect(tokenRoll).toBeDefined()
        expect(tokenRoll?.probability).toBe(0.01)
      }
    })

    it("should replace CRUDE_WEAPON with IMPROVED_WEAPON when dropped", () => {
      // Use a seed that drops improved weapon
      const state = createToyWorld("improved-weapon-drop-seed")
      state.player.location = "MINE"
      state.player.skills.Combat = { level: 1, xp: 0 }
      state.player.inventory.push({ itemId: "CRUDE_WEAPON", quantity: 1 })
      state.player.equippedWeapon = "CRUDE_WEAPON"
    ...
    it("should add COMBAT_GUILD_TOKEN to inventory when dropped", () => {
      const state = createToyWorld("token-drop-test")
      state.player.location = "MINE"
      state.player.skills.Combat = { level: 1, xp: 0 }
      state.player.inventory.push({ itemId: "CRUDE_WEAPON", quantity: 1 })
      state.player.equippedWeapon = "CRUDE_WEAPON"
