...
describe("observation", () => {
  ...
  describe("incremental area discovery", () => {
    /**
     * Helper to set up a world with a known area and frontier areas.
     * After setup:
     * - Player is in TOWN
     * - One distance-1 area is known with a mining node
     * - That area has a known connection to TOWN
     * - That area has connections to distance-2 areas (frontiers)
     */
    async function setupWorldWithFrontiers(seed: string) {
      const state = createWorld(seed)

      // Enrol in Mining guild (discovers a distance-1 area)
      state.exploration.playerState.currentLocationId = "TOWN_MINERS_GUILD"
      await executeAction(state, { type: "Enrol" })
      state.exploration.playerState.currentLocationId = null

      // Find the discovered distance-1 area
      const knownAreaId = state.exploration.playerState.knownAreaIds.find((id) => id !== "TOWN")
      expect(knownAreaId).toBeDefined()

      // Travel to the known area (so we can explore it)
      state.exploration.playerState.currentAreaId = knownAreaId!

      // Discover connections from this area to distance-2 areas (creates frontiers)
      // Find connections from this area to other areas
      const connectionsFromKnownArea = state.exploration.connections.filter(
        (c) => c.fromAreaId === knownAreaId || c.toAreaId === knownAreaId
      )

      // Add a connection to a distance-2 area as known
      for (const conn of connectionsFromKnownArea) {
        const otherId = conn.fromAreaId === knownAreaId ? conn.toAreaId : conn.fromAreaId
        const otherArea = state.exploration.areas.get(otherId)
        if (otherArea && otherArea.distance === 2) {
          const connId = `${conn.fromAreaId}->${conn.toAreaId}`
          if (!state.exploration.playerState.knownConnectionIds.includes(connId)) {
            state.exploration.playerState.knownConnectionIds.push(connId)
          }
          break // Just need one frontier
        }
      }

      // Return to town for consistent starting position
      state.exploration.playerState.currentAreaId = "TOWN"

      return state
    }

    it("produces identical output when discovering new areas incrementally vs full rebuild", async () => {
      const state = await setupWorldWithFrontiers("test-seed-incremental-2")

      // Get initial observation and find a frontier area
      const manager = new ObservationManager()
      const initialObs = manager.getObservation(state)

      expect(initialObs.frontierAreas.length).toBeGreaterThan(0)
      const frontierToVisit = initialObs.frontierAreas[0]

      // Simulate traveling to the frontier (which discovers it)
      // 1. Add the frontier area to known areas
      state.exploration.playerState.knownAreaIds.push(frontierToVisit.areaId)

      // 2. Get the area and add its locations to known locations
      const newArea = state.exploration.areas.get(frontierToVisit.areaId)!
      for (const location of newArea.locations) {
        state.exploration.playerState.knownLocationIds.push(location.id)
      }

      // 3. Add connections from the new area as known
      for (const conn of state.exploration.connections) {
        if (
          conn.fromAreaId === frontierToVisit.areaId ||
          conn.toAreaId === frontierToVisit.areaId
        ) {
          const connId = `${conn.fromAreaId}->${conn.toAreaId}`
          if (!state.exploration.playerState.knownConnectionIds.includes(connId)) {
            state.exploration.playerState.knownConnectionIds.push(connId)
      ...
      state.exploration.playerState.currentAreaId = frontierToVisit.areaId

      // Apply incremental update
      manager.applyActionResult(
        state,
        { type: "Travel", toAreaId: frontierToVisit.areaId },
        {
          ticksConsumed: 22,
          success: true,
          nodesDiscovered: 0,
          areasDiscovered: 1,
          newAreaIds: [frontierToVisit.areaId],
        }
      )
      const incrementalObs = manager.getObservation(state)

      // Build a fresh observation for comparison
      const freshObs = getObservation(state)

      // Compare the two - they should be identical
      const diffs = diffObservations(freshObs, incrementalObs)
      expect(diffs).toEqual([])
    })

    it("correctly updates frontiers when discovering new areas", async () => {
      const state = await setupWorldWithFrontiers("test-seed-frontiers-2")
      ...
      // Simulate traveling to the frontier
      state.exploration.playerState.knownAreaIds.push(frontierToVisit.areaId)
      const newArea = state.exploration.areas.get(frontierToVisit.areaId)!
      for (const location of newArea.locations) {
        state.exploration.playerState.knownLocationIds.push(location.id)
      ...
      // The visited frontier should no longer be in frontiers
      expect(
        updatedObs.frontierAreas.find((f) => f.areaId === frontierToVisit.areaId)
      ).toBeUndefined()

      // The visited area should now be in knownAreas
      expect(updatedObs.knownAreas.find((a) => a.areaId === frontierToVisit.areaId)).toBeDefined()
    })
  })
