...
function evaluateFightAction(state: WorldState, action: FightAction): ActionEvaluation {
...
function evaluateCraftAction(state: WorldState, action: CraftAction): ActionEvaluation {
...
function evaluateStoreAction(state: WorldState, action: StoreAction): ActionEvaluation {
...
function evaluateDropAction(state: WorldState, action: DropAction): ActionEvaluation {
  const item = state.player.inventory.find((i) => i.itemId === action.itemId)
  if (!item || item.quantity < action.quantity) {
    return { expectedTime: 0, expectedXP: 0, successProbability: 0 }
  }
